...
const VideoPlayer = () => {
  ...
  const isFinished = (progress: any) => {
    if (ref.current) {
      ...
      if (currentProgress / duration >= 0.8 && watched != currentId)
        setWatched(currentId);
    }
  };
  ...
  return (
    <div
      className={`course-video w-full h-fit sticky top-0 z-20 ${
        isVideoWide ? "md:sticky" : "md:relative"
      }`}
    >
      <div
        className={`w-full aspect-video ${
          isVideoWide ? "h-[480px]" : "md:aspect-video"
        } overflow-hidden rounded-lg drop-shadow-md`}
      >
        <ReactPlayer
          width={"100%"}
          height={"100%"}
          ...
          url={src}
          controls
          onProgress={(progress) => isFinished(progress)}
        />
      </div>
      <Button
        onClick={toggleWide}
        className={`absolute -bottom-10 right-4 !rounded-full cursor-pointer hidden md:block drop-shadow-md`}
        variant={"outline"}
      >
        {isVideoWide ? <FoldHorizontal /> : <UnfoldHorizontal />}
      </Button>
    </div>
  );
};
